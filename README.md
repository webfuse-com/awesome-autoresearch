...
- [akashgit/remote-factory](https://github.com/akashgit/remote-factory) ![GitHub stars](https://img.shields.io/github/stars/akashgit/remote-factory?style=social) - **The Factory**: Self-evolving, stateful meta-harness for autonomous software development and research. Go from a plain-English idea to a running, continuously improving project. Turn any existing codebase into an autoresearch project in one command — auto-discovers eval dimensions, generates the scoring harness, and starts running keep/revert experiment loops. Research mode wraps any measurable metric into a full optimization loop with failure analysis, monotonic improvement, and leakage guards. Agents evolve their own playbooks from outcomes. Used daily to ship apps, solve math competitions, optimize chip designs, and improve itself.
